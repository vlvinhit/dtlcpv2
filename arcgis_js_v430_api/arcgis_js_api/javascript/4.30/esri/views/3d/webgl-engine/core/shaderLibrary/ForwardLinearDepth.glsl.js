...
}`)}b.ForwardLinearDepth=function(a,f){const {vertex:d}=a;switch(f.output){case c.ShaderOutput.Color:if(f.receiveShadows){g(a);d.code.add(e.glsl`void forwardLinearDepth() { linearDepth = gl_Position.w; }`);return}break;case c.ShaderOutput.Shadow:case c.ShaderOutput.ShadowHighlight:case c.ShaderOutput.ShadowExcludeHighlight:case c.ShaderOutput.ViewshedShadow:a.include(l.VertexPosition,f);g(a);h(a);k(a);d.code.add(e.glsl`void forwardLinearDepth() {
