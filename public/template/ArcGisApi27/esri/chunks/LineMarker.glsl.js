...
function(r,l,t,e,w,x,y,z,A,B,C,D,q,E,u,F,c,G,H,I,J,m,n){function v(a){const b=new H.ShaderBuilder,p=a.hasMultipassTerrain&&(a.output===e.ShaderOutput.Color||a.output===e.ShaderOutput.Alpha),f=a.space===n.LineMarkerSpace.World;b.include(x.RibbonVertexPosition,a);b.include(z.MarkerSizing,a);a.output===e.ShaderOutput.Depth&&b.include(y.OutputDepth,a);const {vertex:d,fragment:k}=b;k.include(D.RgbaFloatEncoding);q.addProjViewLocalOrigin(d,a);b.attributes.add(m.VertexAttribute.POSITION,"vec3");b.attributes.add(m.VertexAttribute.UV0,
"vec2");b.attributes.add(m.VertexAttribute.AUXPOS1,"vec3");b.varyings.add("vColor","vec4");b.varyings.add("vpos","vec3");b.varyings.add("vUV","vec2");b.varyings.add("vSize","float");t.addLinearDepth(b);p&&b.varyings.add("depth","float");a.hasTip&&b.varyings.add("vLineWidth","float");d.uniforms.add(new E.Float2PassUniform("nearFar",(g,h)=>h.camera.nearFar),new u.Float4PassUniform("viewport",(g,h)=>h.camera.fullViewport));d.code.add(c.glsl`vec4 projectAndScale(vec4 pos) {
...
return abs(cos) > 0.001 && t > 0.0;
  ...
  `));q.addPixelRatio(d,a);t.addCalculateLinearDepth(b);d.code.add(c.glsl`void main(void) {
if (uv0.y == 0.0) {
