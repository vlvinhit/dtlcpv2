...
function(p,w,g,E,F,G,H,x,I,J,K,q,L,y,M,r,l,a,N,O,P,m,Q,z){function A(b){const c=new O.ShaderBuilder,{vertex:d,fragment:f}=c,n=b.hasMultipassTerrain&&(b.output===g.ShaderOutput.Color||b.output===g.ShaderOutput.Alpha);c.include(K.PiUtils);c.include(G.RibbonVertexPosition,b);c.include(x.LineStipple,b);const B=b.applyMarkerOffset&&!b.draped;B&&(d.uniforms.add(new l.FloatPassUniform("markerScale",e=>e.markerScale)),c.include(I.MarkerSizing,{space:Q.LineMarkerSpace.World,draped:!1}));b.output===g.ShaderOutput.Depth&&
c.include(H.OutputDepth,b);c.include(F.ObjectAndLayerIdColor,b);y.addProjViewLocalOrigin(d,b);d.uniforms.add(new N.Matrix4PassUniform("inverseProjectionMatrix",(e,h)=>h.camera.inverseProjectionMatrix),new M.Float2PassUniform("nearFar",(e,h)=>h.camera.nearFar),new l.FloatPassUniform("miterLimit",e=>"miter"!==e.join?0:e.miterLimit),new r.Float4PassUniform("viewport",(e,h)=>h.camera.fullViewport));d.constants.add("LARGE_HALF_FLOAT","float",65500);c.attributes.add(m.VertexAttribute.POSITION,"vec3");c.attributes.add(m.VertexAttribute.SUBDIVISIONFACTOR,
"float");c.attributes.add(m.VertexAttribute.UV0,"vec2");c.attributes.add(m.VertexAttribute.AUXPOS1,"vec3");c.attributes.add(m.VertexAttribute.AUXPOS2,"vec3");c.varyings.add("vColor","vec4");c.varyings.add("vpos","vec3");w.addLinearDepth(c);n&&c.varyings.add("depth","float");const t=b.stippleEnabled&&b.stippleScaleWithLineWidth;t&&c.varyings.add("vLineSizeInv","float");const k=b.capType===z.CapType.ROUND,C=t||k;C&&c.varyings.add("vLineWidth","float");const u=b.innerColorEnabled||k;u&&c.varyings.add("vLineDistance",
"float");const D=b.stippleEnabled&&k,v=b.falloffEnabled||D;v&&c.varyings.add("vLineDistanceNorm","float");k&&(c.varyings.add("vSegmentSDF","float"),c.varyings.add("vReverseSegmentSDF","float"));d.code.add(a.glsl`#define PERPENDICULAR(v) vec2(v.y, -v.x);
...
return posNdc;
}`);w.addCalculateLinearDepth(c);d.code.add(a.glsl`
  ...
  `);y.addPixelRatio(d,b);d.code.add(a.glsl`
  void main(void) {
    ...
    if (uv0.y == 0.0) {
      ...
      ${t?a.glsl`vLineSizeInv = 1.0 / lineSize;`:""}
  ...
  `);B&&d.code.add(a.glsl`vec4 other = isStartVertex ? next : prev;
...
joinDisplacementDir = PERPENDICULAR(joinDisplacementDir);`);d.code.add(a.glsl`
      ...
      ${b.capType!==z.CapType.BUTT?a.glsl`capDisplacementDir = isStartVertex ? -right : left;`:""}
    }
  `);d.code.add(a.glsl`
    // Displacement (in pixels) caused by join/or cap
    vec2 dpos = joinDisplacementDir * sign(uv0.y) * displacementLen + capDisplacementDir * displacementLen;

    ${v||u?a.glsl`float lineDistNorm = sign(uv0.y) * pos.w;`:""}

    ${u?a.glsl`vLineDistance = lineWidth * lineDistNorm;`:""}
    ${v?a.glsl`vLineDistanceNorm = lineDistNorm;`:""}
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`),d.uniforms.add(new l.FloatPassUniform("stipplePatternPixelSize",e=>x.computePixelSize(e))),d.code.add(a.glsl`
...
color = mix(color, vec4(innerColor.rgb, innerAlpha), innerAA);`)),f.code.add(a.glsl`vec4 finalColor = blendStipple(color, stippleAlpha);`),b.falloffEnabled&&(f.uniforms.add(new l.FloatPassUniform("falloff",e=>e.falloff)),f.code.add(a.glsl`finalColor.a *= pow(max(0.0, 1.0 - abs(vLineDistanceNorm * gl_FragCoord.w)), falloff);`)));f.code.add(a.glsl`
  ...
  `);return c}const R=Object.freeze(Object.defineProperty({__proto__:null,RIBBONLINE_NUM_ROUND_JOIN_SUBDIVISIONS:1,build:A},Symbol.toStringTag,{value:"Module"}));p.RIBBONLINE_NUM_ROUND_JOIN_SUBDIVISIONS=1;p.RibbonLine=R;p.build=A});