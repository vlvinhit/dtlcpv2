...
function(r,p,w,h,x,y,z,A,B,t,C,u,d,D,E,q){function v(a){const c=new D.ShaderBuilder,m=a.hasMultipassTerrain&&(a.output===h.ShaderOutput.Color||a.output===h.ShaderOutput.Alpha);c.include(y.Transform,a);c.include(w.ScreenSizeScaling,a);c.include(x.SliceDraw,a);const {vertex:k,fragment:f}=c;f.include(B.ColorConversion);t.addProjViewLocalOrigin(k,a);f.uniforms.add(new u.Float4PassUniform("uColor",b=>b.color));c.attributes.add(q.VertexAttribute.POSITION,"vec3");c.varyings.add("vWorldPosition","vec3");
m&&c.varyings.add("depth","float");a.screenSizeEnabled&&c.attributes.add(q.VertexAttribute.OFFSET,"vec3");a.shadingEnabled&&(t.addViewNormal(k),c.attributes.add(q.VertexAttribute.NORMAL,"vec3"),c.varyings.add("vViewNormal","vec3"));k.code.add(d.glsl`
    void main(void) {
