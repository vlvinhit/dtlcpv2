...
function(k,l,h,t,u,m,v,n,g,w,x,d,c,y,z,A){function p(e){const f=new z.ShaderBuilder;f.include(w.ScreenSpacePass,!1);const a=f.fragment;a.uniforms.add(new d.FloatPassUniform("cloudRadius",b=>b.cloudRadius),new d.FloatPassUniform("power",b=>h.lerp(35,120,b.absorption)),new d.FloatPassUniform("sigmaE",b=>1+b.absorption),new d.FloatPassUniform("density",b=>h.lerp(0,.3,b.density)),new d.FloatPassUniform("cloudSize",b=>h.lerp(0,.02,Math.max(.01,1-b.cloudSize))),new d.FloatPassUniform("detailSize",b=>h.lerp(0,
.2,Math.max(.01,1-b.detailSize))),new d.FloatPassUniform("smoothness",b=>h.lerp(0,.5,1-b.smoothness)),new d.FloatPassUniform("cloudHeight",b=>h.lerp(0,1500,b.cloudHeight)),new d.FloatPassUniform("coverage",b=>b.coverage),new y.Matrix3DrawUniform("view",b=>b.viewMatrix),new A.Texture2DPassUniform("cloudShapeTexture",b=>null!=b.noiseTexture?b.noiseTexture.textureAtlas:null),new x.Float2PassUniform("cloudVariables",b=>u.set(B,b.coverage,b.absorption)));a.constants.add("halfCubeMapSize","float",.5*e.cubeMapSize);
a.code.add(c.glsl`
    ...
    float getCloudMap(vec2 p){
      ...
      vec2 uv = (${c.glsl.float(g.WEATHER_MAP_SCALE)} * p) / ${c.glsl.float(g.ATLAS_SIZE)} + 0.5;
...
fragColor = vec4(shading, totalTransmittance, shading, totalTransmittance);
}`);return f}let q=function(e){function f(){var a=e.apply(this,arguments)||this;a.cloudRadius=0;a.cloudSize=0;a.detailSize=0;a.absorption=0;a.density=0;a.smoothness=0;a.cloudHeight=0;a.coverage=0;a.raymarchingSteps=v.cloudPresets.default.raymarchingSteps;a.weatherTile=m.create();return a}l._inherits(f,e);return l._createClass(f)}(c.NoParameters),r=function(e){function f(){var a=e.apply(this,arguments)||this;a.viewMatrix=t.create();return a}l._inherits(f,e);return l._createClass(f)}(c.NoParameters);
const B=m.create(),C=Object.freeze(Object.defineProperty({__proto__:null,CloudsDrawParameters:r,CloudsPassParameters:q,build:p},Symbol.toStringTag,{value:"Module"}));k.Clouds=C;k.CloudsDrawParameters=r;k.CloudsPassParameters=q;k.build=p});