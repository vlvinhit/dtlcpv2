...
function(m,n,h,u,v,w,x,y,z,k,l,p,d,A,B){function q(a){const e=new A.ShaderBuilder,{vertex:f,fragment:g}=e;f.include(w.AlignPixel);e.include(x.HUD,a);e.include(v.SliceDraw,a);e.attributes.add(B.VertexAttribute.UV0,"vec2");f.uniforms.add(new l.Float4PassUniform("viewport",(b,c)=>c.camera.fullViewport),new p.FloatPassUniform("lineSize",(b,c)=>0<b.size?Math.max(1,b.size)*c.camera.pixelRatio:0),new k.Float2PassUniform("pixelToNDC",(b,c)=>n.set(r,2/c.camera.fullViewport[2],2/c.camera.fullViewport[3])),
new p.FloatPassUniform("borderSize",(b,c)=>null!=b.borderColor?c.camera.pixelRatio:0),new k.Float2PassUniform("screenOffset",(b,c)=>n.set(r,b.screenOffset[0]*c.camera.pixelRatio,b.screenOffset[1]*c.camera.pixelRatio)));e.varyings.add("coverageSampling","vec4");e.varyings.add("lineSizes","vec2");a.hasMultipassGeometry&&e.varyings.add("depth","float");a.hasScreenSizePerspective&&z.addScreenSizePerspectiveAlignment(f);f.code.add(d.glsl`
    void main(void) {
  ...
  `);g.uniforms.add(new l.Float4PassUniform("uColor",b=>t(b.color)),new l.Float4PassUniform("borderColor",b=>t(b.borderColor)));a.hasMultipassGeometry&&(g.include(y.multipassGeometryTest,a),g.uniforms.add(new k.Float2PassUniform("inverseViewport",(b,c)=>c.inverseViewport)));g.code.add(d.glsl`
    void main() {
