...
function(h,k,f,w,l,m,x,y,z,A,B,C,D,n,E,F,g,p,G,H,e,q){function r(c){const d=new H.ShaderBuilder;d.include(B.ReadShadowMapDraw,c);c=d.fragment;c.include(D.RgbaFloatEncoding);c.include(A.ReadLinearDepth);d.include(C.CameraSpace);d.include(y.ScreenSpacePass);c.uniforms.add(new e.Texture2DPassUniform("defaultDepthTex",(b,a)=>a.shadowMap.getSnapshot(q.SnapshotSlot.Default)),new e.Texture2DPassUniform("highlightDepthTex",(b,a)=>a.shadowMap.getSnapshot(q.SnapshotSlot.Highlight)),new e.Texture2DPassUniform("depthMap",
(b,a)=>a.linearDepthTexture),new e.Texture2DPassUniform("highlightMap",(b,a)=>a.highlightColorTexture),new F.Float4PassUniform("uColor",b=>b.shadowColor),new n.Float2PassUniform("nearFar",(b,a)=>a.camera.nearFar),new g.FloatPassUniform("opacity",b=>b.shadowOpacity),new g.FloatPassUniform("occludedOpacity",b=>b.occludedShadowOpacity),new g.FloatPassUniform("terminationFactor",b=>b.opacityElevation*b.dayNightTerminator),new E.Float3PassUniform("lightingMainDirectionView",(b,a)=>m.normalize(t,m.transformMat4(t,
a.lighting.mainLight.direction,a.camera.viewInverseTransposeMatrix))),new n.Float2PassUniform("texelSize",(b,a)=>null!=a.linearDepthTexture?w.set(I,1/a.linearDepthTexture.descriptor.width,1/a.linearDepthTexture.descriptor.height):l.ZEROS),new G.Matrix4PassUniform("inverseViewMatrix",(b,a)=>k.invert(u,k.translate(u,a.camera.viewMatrix,a.camera.center))));c.constants.add("unoccludedHighlightFlag","vec4",z.unoccludedHighlightFlag).add("highlightedThreshold","float",v.highlightedThreshold).add("selfShadowThreshold",
...
}`);return d}const v={highlightedThreshold:.99999,selfShadowThreshold:.025},u=f.create(),t=x.create(),I=l.create();f=Object.freeze(Object.defineProperty({__proto__:null,build:r},Symbol.toStringTag,{value:"Module"}));h.ShadowHighlight=f;h.build=r});